...
def backup_system(backup_path):
    ...
    try:
        ...
        home = str(Path.home())
        for config_file in [".bashrc", ".zshrc", ".vimrc", ".config/i3/config", 
                           ".config/sway/config", ".xinitrc", ".Xresources"]:
            ...
            for path in backup_paths:
                if os.path.exists(path):
                    ...
                    if path.startswith(home):
                        arcname = os.path.join("home_config", os.path.relpath(path, home))
                    else:
                        arcname = path.lstrip("/")
...
def optimize_system():
    ...
    try:
        ...
        if orphans.stdout:
            ...
            optimizations_applied.append(f"Removed {len(orphan_list)} orphaned packages")
        else:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if restore_packages:
                ...
                pkg_list_files = [f for f in os.listdir(backup_dir) if f.startswith("installed_packages_") and f.endswith(".txt")]
                ...
                if pkg_list_files:
                    ...
                    for i in range(0, len(packages), batch_size):
                        ...
                        try:
                            logger.info(f"Installing package batch {i//batch_size + 1}/{(len(packages)-1)//batch_size + 1}")
                            subprocess.run(["pacman", "-S", "--needed", "--noconfirm"] + batch, check=True)
                        except:
                            logger.warning(f"Failed to install some packages in batch {i//batch_size + 1}")
                else:
                    logger.warning("No package list file found, skipping package restoration")
