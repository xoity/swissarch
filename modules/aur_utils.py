...
def install_aur_helper(helper_name):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            subprocess.run(["pacman", "-S", "--needed", "--noconfirm", "git", "base-devel"], check=True)
    ...
    try:
        ...
        if helper:
            ...
            if result.returncode == 0:
                ...
                logger.warning(f"AUR search returned non-zero exit code: {result.returncode}")
                return result.stdout + "\n" + result.stderr
        else:
            # Fallback to manual curl request to AUR RPC
            import requests
            import json
            ...
            result = f"Found {data['resultcount']} packages in AUR matching '{query}':\n\n"
...
def install_aur_package(package_name, helper=None):
    ...
    try:
        ...
        if helper and not shutil.which(helper):
            logger.warning(f"{helper} not found, will try to use any available AUR helper")
            helper = None
        ...
        if helper:
            result = subprocess.run([helper, "-S", "--noconfirm", package_name], check=True)
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                subprocess.run(["git", "clone", f"https://aur.archlinux.org/{package_name}.git"], check=True)
